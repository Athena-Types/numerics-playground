\section{Appendix: Subsitution lemma and metric preservation proof}
\begin{lemma}[$r$-sensitive subsitution]
...
For the metric preservation proof, we treat our distance vector $\gamma$ as a
lookup function and assume that there is an implicit fixed ordering on the
variables.
...
The below two lemmas are helpful towards proving metric preservation:
...
\section{Appendix: Proof that Negative Fuzz adheres to the modular interface} 
...
and application of triangle inequality with respect to $d_{\mathbb{R}}$.
\end{proof}
...
round-off error, recall that we define a ``simulated" rounding function
...
\section{Appendix: Error bound translation theorems}
\begin{theorem}[Paired error theorem; relative]
...
\begin{theorem}[Paired error theorem; absolute] 
  For any numbers $((r, a, b), (r', a', b')) \in \mathbb{P}^2$ and where we have
...
\section{Appendix: Helper functions for defining type inference.}
\begin{figure}
...
\end{subfigure}
    ...
    \caption{Ancillary functions defining algorithmic type inference. I creates
      bounds for an type with erased bounds. E extracts the bounds used in an
      unerased type. For type checking to succeed on many real-world programs,
      it is important for I and E to align in the production and extraction of
    bound variables.}
...
\section{Appendix: Soundness proof for algorithmic type inference} 
...
\begin{theorem}[Algorithmic Soundness] 
