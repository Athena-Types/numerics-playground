...
\begin{definition}[Logical relation]
  ...
  \begin{aligned}[c]
    ...
    \mathcal{VR_{\mathbf{\tau_0 \multimap \tau_1}}} &\triangleq 
      \{ v \ | \ \forall w_0, w_1 \in \mathcal{VR}_{\tau_0}, \ v~w_0, v~w_1 \in
      \mathcal{R}_{\tau_1} \text{ and } \mathcal{SD}_{\tau_1}(v~w_0, v~w_1) \leq
...
\begin{lemma}[$\mathcal{SD}$ is a metric]
  ...
  \begin{enumerate}
    \item Distance from any point to itself is zero: $\forall x,~\mathcal{SD}(x,
      x) = 0$.
    \item Positivity: $\forall x, y,~\mathcal{SD}(x, y) > 0$.
...
\end{lemma}
\begin{proof}
  The properties holds for the base cases of $\mathcal{SDV}$ and follows for the
  remaining cases by our inductive hypothesis. Since our operational semntics is
  deterministic, the properties follows for $\mathcal{SD}$.
\end{proof}
