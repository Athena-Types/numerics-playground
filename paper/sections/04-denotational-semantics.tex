...
appropriate $\rho$ and equipped with the appropriate set of numeric computations
$\Sigma$.  
...
\begin{description}
  \item[a) Property of $\rho$ and $\mathbf{rnd}$.] We assume that the $\forall e
    \in \mathcal{R}_{num}, \mathcal{SD}_{\mathbf{num}}(\rho(e), e) \leq q$ where
    $q$ is the grade in the $\mathbf{rnd}$ typing rule.
  \item[b) Property of $\mathbf{op}$.] We also assume that for every operation
    $\mathbf{op} : \tau_0 \multimap \tau_1 \ \in \ \Sigma$ the following two
    properties hold:
    \begin{description}
      ...
      \item[\underline{2) Sensitivity.}] For any $e_1, e_2 \in \mathcal{R}_{\tau_0}$ that
        $\mathcal{SD}_{\tau_1}(\mathbf{op}(e_1), \mathbf{op}(e_2))$.
    \end{description}
\end{description}
\end{definition}
