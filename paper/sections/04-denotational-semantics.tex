...
\begin{definition}[Machine configuration interpretation]
  ...
  \llbracket \sigma_0 \Vdash v \rrbracket_{\tau_0 \multimap \tau_1}
    ...
    \rrbracket_{\tau_0} \not= \bot \} \\
  \llbracket \sigma \Vdash v \rrbracket_{!_s \tau} &\triangleq \llbracket \sigma
    \Vdash v \rrbracket_\tau \\
  \llbracket \sigma \Vdash v \rrbracket_{\tau_0 + \tau_1} &\triangleq
    \begin{cases}
      (\llbracket \sigma \Vdash v \rrbracket_{\tau_0}, 0) \quad &\text{if well-defined} \\
      (0, \llbracket \sigma \Vdash v \rrbracket_{\tau_1}) \quad &\text{if well-defined}
    \end{cases} \\
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %%%            Tree cases             %%%
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  \end{aligned}
  \end{equation}
  This definition is well-founded. It is defined first over types. It is then
  defined on values and enviroment leafs, with $\tau$ decreasing or $\tau$
  constant and $\gamma$ decreasing in size. Finally, it is defined on arbitrary
  enviroment trees and then on expressions that are not values, if it can be
