...
division or taking square roots; we leave this as a potential future direction.
...
our running example of pairwise summation on $w,x,y,z \in [-1, 1]$.} FPTaylor
and Gappa occupy a different part of the performance-precision design space
comapred to Negative Fuzz: both FPTaylor and Gappa aim for precise, sound
...
Negative Fuzz is frequently orders of magnitude faster than Gappa and FPTaylor.
...
benchmarks, which we discuss below. Our large-scale comparions are primarly
...
analysis time. We include Gappa and FPTaylor to demonstratate that those two
...
\begin{enumerate}
  \item We evaluate the Horner polynomial evaluation scheme on polynomials of
    degree $2^2$ to $2^{11}$ and we observe that Negative Fuzz has comparable
    precision at significantly faster performance.
  \item We evaluate matrix multiplication on matricies of $2^2 \times 2^2 $ to
    $2^7 \times 2^7$. On large inputs, we observe that Negative Fuzz (without
    \textbf{factor}) comparable precision at significantly faster performance.
  ...
  \item We evaluate the iterative summation algorithm (first discussed in
    Section~\ref{sec:overview}) on sequences of addition from $2^2$ to $2^7$. We
    observe that Negative Fuzz has comparable precision at significantly faster
    performance.
\end{enumerate}
