...
:_s \tau; x; \tau}
\end{equation}
We induct over each typing / case in the full algorithm. 

TODO
\end{proof}

\begin{lemma}[Empty context] \label{thm:empty-context}
If $\Delta \ | \ . \vdash e : \tau$ has a valid derivation, then $. \vdash e :
\tau$ has a valid derivation.
\end{lemma}

\begin{lemma}[Soundness from empty context]
If $.; e^{\bullet} \implies .; e'; \tau'$, then $. \vdash e : \tau'$ has a valid
derivation.
\end{lemma}
\begin{proof}
Follows by application of Theorem~\ref{thm:algo-soundness} and
Lemma~\ref{thm:empty-context}.
\end{proof}
