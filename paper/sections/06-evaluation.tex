...
\AXC{}
\RightLabel{(Unit)}
\UIC{$\Gamma; \langle \rangle^{\bnd{\bullet}}; \implies \Gamma; \langle \rangle; \textbf{unit}$}
\bottomAlignProof
\DisplayProof
\hskip 0.5em
%% var
\AXC{}
\RightLabel{(Var)}
\UIC{$\Gamma, x :_s \tau, x^{\bnd{\bullet}} \implies \Gamma, x :_s \tau; x; \tau$}
\bottomAlignProof
\DisplayProof
\vskip 1em
%% bang intro
\AXC{$\Gamma; e^{\bnd{\bullet}} \implies \Gamma; e; \tau$}
\RightLabel{($!$ I)}
\UIC{$s * \Gamma; [e]_{\annotate{s}}^{\bnd{\bullet}} \implies s * \Gamma;
[e]_{\annotate{s}}; !_s~\tau$}
\bottomAlignProof
\DisplayProof
\vskip 1em
...
\AXC{$\Gamma, x : \tau_0; e^{\bnd{\bullet}} \implies \Gamma, x :_s \tau_0; e;
\tau$}
% \AXC{$\epsilon_0, \ldots, \epsilon_n \not\in FTV(\Gamma) \cup DOM(\Delta)$}
\RightLabel{($\multimap$ I)}
\BIC{$\Gamma; \lambda (x : \annotate{\tau_0^{\bnd{\bullet}}}). e^{\bnd{\bullet}}
\implies \Gamma; \Lambda \epsilon_0, \ldots, \epsilon_n (\lambda (x : \tau_0) .
e); \forall \epsilon_0, \ldots, \epsilon_n . \ \tau_0 \multimap \tau$}
\bottomAlignProof
\DisplayProof
\vskip 1em
...
\AXC{$
\begin{aligned}[t]
\Theta; f^{\bnd{\bullet}} &\implies \Theta; f; \forall \epsilon_0, \ldots,
\epsilon_n \ . \ \tau_0 \multimap \tau \\
\Gamma; e^{\bnd{\bullet}} &\implies \Gamma; e; \tau'_0 \\ \end{aligned}
$}
\hskip 0.5em
\AXC{$\begin{aligned}[t]
...
$}
\RightLabel{($\multimap$ E)}
\TIC{$\Gamma + \Theta; (f~e)^{\bnd{\bullet}} \implies \Gamma + \Theta; (f~\{ i_0
\}~\ldots~\{ i_n \})~e; \tau'$}
\bottomAlignProof
\DisplayProof
\vskip 1em
...
\UIC{$\Gamma; \textbf{op}^{\bnd{\bullet}} 
\implies \Gamma; \textbf{op}; \tau$}
\bottomAlignProof
\DisplayProof
\vskip 1em

\RightLabel{(\textbf{factor})}
\AXC{$\Gamma; e^{\bnd{\bullet}} \implies \Gamma; e; (M_q \tau_0 \times M_r
\tau_1)$}
\UIC{$\Gamma; \textbf{factor}~e^{\bnd{\bullet}} \implies \Gamma;
\textbf{factor}~e; M_{max(q, r)} (\tau_0 \times \tau_1)$}
\bottomAlignProof
\DisplayProof
\vskip 1em
...
\UIC{$\Gamma; \textbf{ret}~e^{\bnd{\bullet}} \implies \Gamma; \textbf{ret}~e;
M_0~\tau$}
\bottomAlignProof
\DisplayProof
\hskip 0.5em
...
\UIC{$\Gamma; \textbf{rnd}~e^{\bnd{\bullet}} \implies \Gamma; \textbf{rnd}~e;
M_q~\tau$}
\bottomAlignProof
\DisplayProof
\vskip 1em
...
%% times intro 
\AXC{$\Gamma; e^{\bnd{\bullet}} \implies \Gamma; e; \tau_0$}
\AXC{$\Gamma; f^{\bnd{\bullet}} \implies \Gamma; f; \tau_1$}
% \AXC{$
% \begin{aligned}[t]
...
% $}
\RightLabel{($\times$ I)}
\BIC{$\Gamma; \langle f, e \rangle^{\bnd{\bullet}} \implies \Gamma; \langle f, e
\rangle; \tau_0 \times \tau_1$}
\bottomAlignProof
\DisplayProof
\RightLabel{($\times$ E)}
\AXC{$\Gamma; e^{\bnd{\bullet}} \implies \Gamma; e; \tau_0 \times \tau_1$}
\UIC{$\Gamma; (\pi_i~e)^{\bnd{\bullet}} \implies \Gamma; \pi_i~e; \tau_i$}
\bottomAlignProof
\DisplayProof
\vskip 1em
...
%% otimes intro 
\AXC{$\Gamma; e^{\bnd{\bullet}} \implies \Gamma; e; \tau_0$}
\AXC{$\Theta; f^{\bnd{\bullet}} \implies \Theta; f; \tau_1$}
\RightLabel{($\otimes$ I)}
\BIC{$\Gamma + \Theta; \langle f, e \rangle^{\bnd{\bullet}} \implies \Gamma +
\Theta; (f, e); \tau_0 \otimes \tau_1$}
\bottomAlignProof
\DisplayProof
\vskip 1em
...
\AXC{$\Gamma; e^{\bnd{\bullet}} \implies \Gamma; e; \tau_0$}
\AXC{$\Theta, x; f^{\bnd{\bullet}} \implies \Theta, x :_s \tau_0; \tau$}
\BIC{$s * \Gamma + \Theta; (\textbf{let} \ x \ = \ e \ \textbf{in} \ f)^{\bnd{\bullet}}
\implies s * \Gamma + \Theta; \textbf{let} \ x \ = \ e \ \textbf{in} \ f; \tau$}
\bottomAlignProof
\DisplayProof
\vskip 1em

\RightLabel{(\textbf{let-pair})}
\AXC{$\Gamma; e^{\bnd{\bullet}} \implies \Delta_0 \ | \ \Gamma;
e; \tau_0 \otimes \tau_1$}
\AXC{$\Theta, x, y; f^{\bnd{\bullet}} \implies \Delta_1 \ | \ \Theta, x :_s
\tau_0, y :_s \tau_1; f; \tau$}
\BIC{$s * \Gamma + \Theta; (\textbf{let-pair} \ (x, y) \ = \ e \ \textbf{in} \
f)^{\bnd{\bullet}} \implies s * \Gamma + \Theta; \textbf{let-pair} \ (x, y) \ =
\ e \ \textbf{in} \ f; \tau$}
\bottomAlignProof
\DisplayProof
\vskip 1em
...
\AXC{$\Gamma; e^{\bnd{\bullet}} \implies \Gamma; e; M_r~\tau_0$}
\AXC{$\Theta, x:_s \tau_0; f^{\bnd{\bullet}} \implies \Theta, x :_s \tau_0;
M_q~\tau$}
\BIC{$s * \Gamma + \Theta; (\textbf{let-bind} \ x \ = \ e \ \textbf{in} \
f)^{\bnd{\bullet}} \implies s * \Gamma + \Theta; \textbf{let-bind} \ x \ = \ e \
\textbf{in} \ f; M_{s*r + q}~\tau$}
\bottomAlignProof
\DisplayProof
\vskip 1em

\RightLabel{(\textbf{case})}
\AXC{$\Gamma; e^{\bnd{\bullet}} \implies \Gamma; e; \tau_0 + \tau_1$}
\AXC{$\Theta, x :_s \tau_0 ; f_1^{\bnd{\bullet}} \implies \Theta, x :_s \tau_0;
f_1; \tau$}
\AXC{$\Theta, x :_s \tau_0 ; f_2^{\bnd{\bullet}} \implies \Theta, x :_s \tau_0;
f_2; \tau$}
\TIC{$s * \Gamma + \Theta; (\textbf{case} \ e \ \textbf{of} \
(\textbf{in}_1~x.f_1 \ | \ \textbf{in}_2~x.f_2))^{\bnd{\bullet}} \implies s *
\Gamma + \Theta; (\textbf{case} \ e \ \textbf{of} \ (\textbf{in}_1~x.f_1 \ | \
\textbf{in}_2~x.f_2)) ; M_{s*r + q}~\tau$}
\bottomAlignProof
\DisplayProof
\vskip 1em
