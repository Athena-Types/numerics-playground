...
$}
\RightLabel{($\multimap$ E)}
\TIC{$\Gamma + \Theta; (f~e)^{\bnd{\bullet}} \implies \Gamma + \Theta; (f'~\{
  i_0 \}~\ldots~\{ i_n \})~e'; \tau'$}
\bottomAlignProof
\DisplayProof
\vskip 1em
...
% $}
\RightLabel{($\times$ I)}
\BIC{$\Gamma; \langle f, e \rangle^{\bnd{\bullet}} \implies \Gamma; \langle f',
e' \rangle; \tau_0 \times \tau_1$}
\bottomAlignProof
\DisplayProof
\RightLabel{($\times$ E)}
...
\AXC{$\Gamma; e^{\bnd{\bullet}} \implies \Gamma; e'; \tau_0$}
\AXC{$\Theta, x; f^{\bnd{\bullet}} \implies \Theta, x :_s \tau_0; f'; \tau$}
\BIC{$s * \Gamma + \Theta; (\textbf{let} \ x \ = \ e \ \textbf{in} \
f)^{\bnd{\bullet}} \implies s * \Gamma + \Theta; \textbf{let} \ x \ = \ e' \
\textbf{in} \ f'; \tau$}
\bottomAlignProof
\DisplayProof
\vskip 1em
...
\AXC{$\Gamma; e^{\bnd{\bullet}} \implies \Delta_0 \ | \ \Gamma; e'; \tau_0
\otimes \tau_1$}
...
\AXC{$\Theta, x:_s \tau_0; f^{\bnd{\bullet}} \implies \Theta, x :_s \tau_0; f;
M_q~\tau$}
...
\Gamma + \Theta; (\textbf{case} \ e' \ \textbf{of} \ (\textbf{in}_1~x.f'_1 ~ | ~
\textbf{in}_2~x.f'_2)) ; M_{s*r + q}~\tau$}
\bottomAlignProof
\DisplayProof
\vskip 1em
