...
As language designers, we wish to convince the ordinary programmer that the
floating-point semantics we have provided is ``close enough" to what they
desire.
...
(1) how a function magnifies error in its inputs (function sensitivity), and (2)
the maximum error introduced by the function. By simultaneously bounding these
two properties, Numerical Fuzz is able to compositionally bound the maximum
accmulated error in a program via a graded monad.
...
The limitations of Numerical Fuzz we address are orthogonal, so we describe them
separately. To support subtraction, we first introduce a \textit{paired
representation} that simulates the floating-point error of a standard, unpaired
program representation.
We demonstrate how we can encode subtraction as a finitely-sensitive operation.
Then, we use a bounds analysis to translate the simulated error  on the paired
representation into floating-point error on the standard representation.
Finally, we introduce a new primitive that can more precisely capture the
...
\paragraph{Supporing subtraction and negative numbers.} 
% TODO: polish and org this entire para
To support subtraction and negative numbers, we represent each possibly negative
real, $r$, in our semantics with a pair of non-negative $(a, b)$ which behave
akin to ghost variables.
...
to add components and is always-growing; subtraction is represented implictly.
Distance between $(a_0, b_0)$ and $(a_1, b_1)$ is encoded as the maximum of the
pairwise distance: $max(d(a_0, a_1), d(b_0, b_1))$. 
By ensuring that the paired representation is always-growing, we avoid the
problem of ill-conditioned subtraction and can encode subtraction in a
finite-sensitive manner. 
...
To accomplish this, we incorporate a bounds analysis in the type system. A
challenge of doing a bounds analysis in the type system is how to assign a
precise bound to a function that has multiple call sites. A na{\"i}ve approach
might widen the bounds for each call site, drastically reducing the precision of
the bounds analysis. We address this challenge by introducing \textit{bound
polymorphism}, allowing functions to be polymorphic in the bounds assigned to
the inputs. At each call site, we specialize the polymorphic function to the
appropriate bounds. 
...
\paragraph{More precise analysis of addition and subtraction.}
% TODO: fill in factor stuff, intuition and ref
...
analysis, such as Numerical Fuzz \cite{numfuzz} and Bean \cite{bean}. 
There is no need to perform global
optimization, run an algorithm to convergence, saturate rewrites, or bit-blast.
