...
\RightLabel{($\forall$-E)}
\BIC{$\Delta \ | \ \Gamma \vdash e~\{b\} : \tau[b/\epsilon]$}
\bottomAlignProof
\DisplayProof
...
\AXC{$\textbf{op}_c \in \Sigma_{\textbf{bnd}}$}
\RightLabel{($\textbf{bnd}$-\textbf{op})}
\QIC{$\Delta \vdash \textbf{op}_c(i_0, \ldots, i_c) : \textbf{bnd}$}
\bottomAlignProof
\DisplayProof
...
\end{center}
    ...
    constant). $FTV(\Gamma)$ refers to all the free type variables in $\Gamma$.}
