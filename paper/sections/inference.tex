...
\begin{lemma}[Well-formedness of intervals produced by $E$.] \label{thm:wff-var-e}
  ...
  $i_j$, $FTV(\tau) = \Delta \vdash i_j$.
\end{lemma}
\begin{proof}
  By induction over types $\tau$.
\end{proof}
...
\begin{description}
  ...
  \item[$\multimap$ I.] By our inductive hypothesis, we know that there exists a
    ...
    $\forall-I$ rule to $d$. For all $i$ (each application), we must satisfy that
  ...
  \item[$\multimap$ E.] By our inductive hypothesis, we know that there exists a
    $\Delta'$ such that we have a derivation $d_0$ where
    $\Delta' \ | \ \Gamma' \vdash f : \forall \epsilon_0, \cdots, \epsilon_n,
    \tau_0 \multimap \tau$
    and $\Delta''$ such that we have a derivation $d_1$ where
    $\Delta'' \ | \ \Theta \vdash e : \tau_0'$.
    By Lemma~\ref{thm:free-var-gamma}, we have that 
    $FTV(\tau'_0) \subseteq FTV(\Theta) \subseteq \Delta''$.
    ...
    By weakeing, we have a derivation for $\Delta \ | \ \Theta \vdash e :
    \tau_0[\epsilon_0 / i_0, \cdots, \epsilon_n / i_n]$ (think contrapositively
    for now). 
    Then we are able to
    ...
    Note that the $\Delta \vdash i : \textbf{bnd}$ condition in each applicatin
    of the $\forall E$ rule is satisfied by Lemma~\ref{thm:wff-var-e} and
    Lemma~\ref{thm:free-var-gamma} and weakening on the interval bound
    judgements.
