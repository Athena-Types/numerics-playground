...
also modular. We parametrize our algorithm by an abstraction function mapping
from our concrete domain $\textit{num}$ to intervals: $\alpha : \textit{num} \to
\textit{num}^2$. 
We only constrain $\alpha$ to be closed:
\begin{definition}[$\alpha$ is closed.] \label{def:alpha-closed}
For all $k \in \textit{num}$, $k \in \alpha(k)$.
\end{definition}
...
\begin{enumerate}
  ...
  $$\Gamma; e^{\bnd{\bullet}} \underset{bnd.}{\implies} \Gamma; e'; \tau'$$
    During bound inference, we rely on helper functions $I$ and $E$
...
\AXC{}
\RightLabel{(Const)}
\UIC{$\Gamma; k^{\bnd{\bullet}}; \underset{bnd.}{\implies} \Gamma;
k; \textbf{num}_{\bnd{\alpha(k)}}$}
\bottomAlignProof
\DisplayProof
\hskip 0.5em
%% bang intro
...
\tau_0; f'; \tau$}
\BIC{$s \cdot \Gamma + \Theta; (\textbf{let} \ x \ = \ e \ \textbf{in} \
f)^{\bnd{\bullet}} \underset{bnd.}{\implies} s \cdot \Gamma + \Theta; \textbf{let} \
x \ = \ e' \ \textbf{in} \ f'; \tau$}
\bottomAlignProof
\DisplayProof
\vskip 1em
...
\TIC{$s \cdot \Gamma + \Theta; (\textbf{case} \ e \ \textbf{of} \
(\textbf{in}_1~x.f_1 ~ | ~ \textbf{in}_2~x.f_2))^{\bnd{\bullet}}
\underset{bnd.}{\implies} s \cdot \Gamma + \Theta; (\textbf{case} \ e' \ \textbf{of}
\ (\textbf{in}_1~x.f'_1 ~ | ~ \textbf{in}_2~x.f'_2)) ; M_{s \cdot r + q}~\tau$}
\bottomAlignProof
\DisplayProof
\vskip 1em
...
\begin{description}
  \item[Const.] Holds by our interface for $\alpha$ from
  Definition~\ref{def:alpha-closed}. 
...
% $\vdash_{\mathbb{R}}$ judgement. We observe that programs step the same in both
% instantiations of the language and also that programs derivable in
% $\vdash_{\mathbb{R}}$ have the same type with $\vdash$.
...
%   set of $\mathbb{P}$ is zeroed out in the last component. Therefore, $b = b' =
%   b^{\uparrow} = b^{\downarrow} = 0$. By applying our a priori error theorem
