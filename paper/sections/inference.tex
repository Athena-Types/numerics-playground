...
also modular and parameterized by an abstraction function mapping from our
concrete domain $\textit{num}$ to intervals: $\alpha : \textit{num} \to
\textit{num}^2$. 
We only place the following constraint on $\alpha$:
...
bounds by adding the following case:
