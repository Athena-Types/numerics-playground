...
perform the type sensitivity and inference algorithm developed in
\cite{numfuzz}, extended to our core language without bound polymorphism.
We also automatically infer polymorphic bound annotations to generate a valid
typing derivation of the program in \bnd{\Lang}.
...
\end{lemma}
\begin{proof}
  We apply the contrapositive of weakening (over bound contexts,
  Lemma~\ref{thm:weakening-bnd}) to the
  conclusion. So it suffices to prove that $FTV(\tau) \vdash i_j$.
  We complete the proof by induction over $\tau$.
\end{proof}
...
\begin{description}
  ...
  \item[$\multimap$ E.] By our inductive hypothesis, we know that there exist
    ...
    By weakening on our inducitve hypothesis, we have a derivation for $\Delta \
  ...
  \item[$\times$ I.] Like Numerical Fuzz, we define \text{max} as the least
    ...
    f', e' \rangle; \tau_0 \times \tau_1$.
  \item[$\times$ E.] Holds trivially.
  \item[$\times$ E.] Holds trivially.
  \item[$\otimes$ I.] Holds trivially.
  \item[Let.] Holds trivially.
  \item[Let-pair.] Holds by weakening and the inductive hypothesis.
  ...
  \item[$\otimes$ E.] By the definition of $\text{max}$, we have that 
    ...
    We can now finish by applying our $\otimes$ E typing rule.
\end{description}
\end{proof}
