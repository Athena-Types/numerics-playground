...
\RightLabel{($\tensor$ E)}
\BIC{$s * \Gamma + \Theta \vdash \letdestruct (x,y) \ = \ e \ \tin \ f : \rho $}
\bottomAlignProof
\DisplayProof
\vskip 1em
%%
...
% factor
\AXC{$\Gamma \vdash e : \langle M_q \tau, \ M_r \sigma \rangle$}
%\AXC{$r + q \leq s$}
%\AXC{$s = max(r,q)$}
\RightLabel{(Factor)}
\UIC{$\Gamma \vdash \factor \ e : M_{max(q,r)} \langle \tau, \sigma \rangle$}
\bottomAlignProof
\DisplayProof
