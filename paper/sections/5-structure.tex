...
computation tree. Recall our running example from Section~\ref{sec:overview},
computing addition for four variables: $w \tilde{+} x \tilde{+} y \tilde{+} z$.
Consider the following two ways to associate the expression:
\begin{description}
  \item[1) The degenerate way.] The abstract syntax tree here resembles the
    structure of a linked-list: 
    $$((w \tilde{+} x) \tilde{+} y) \tilde{+} z$$
    Error terms grow in $O(n)$, where $n$ is the number of subterms.
  \item[2) The balanced tree way.] The abstract syntax tree represents a
    balanced binary tree:
    $$(w \tilde{+} x) \tilde{+} (y \tilde{+} z)$$
    Error terms grow in $O(ln(n))$.
\end{description}
...
We wish for our language to be sensitive to the associativity of floating-point
operations.
In Figure~\ref{fig:fail-assoc-side-by-side}, we consider how to write both
expressions in Negative Fuzz without \textbf{factor}. Note that the monadic
error grade for both computations is the same.
\begin{figure}[ht]
\centering
...
let-bind c = addfp <b, z> in
addfp c 
\end{lstlisting}
\caption{Program for the degenerate tree computation.}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.48\textwidth}
\begin{lstlisting}
...
addfp c 
\end{lstlisting}
\caption{Program for the balanced tree computation.}
\end{subfigure}

\caption{Side-by-side comparison of two ways to associate the $w \tilde{+} x
\tilde{+} y \tilde{+} z$ computation.}
\label{fig:fail-assoc-side-by-side}
\end{figure}
...
function:
\footnote{For the remainder of this section, we omit the bound subscripts.}
...
\DisplayProof
\end{figure}
is the only way to eliminate the monadic $M_q \tau$ type. Consider the
...
the monadic bind rule in the \textit{typing tree} does not differ. The Negative
