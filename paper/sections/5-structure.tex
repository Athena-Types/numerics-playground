...
\textbf{factor} that improves the precision of addition. We then show how to use
\textbf{factor} with subtraction. Next, we discuss how rewriting a program to
use \textbf{factor} is entirely orthogonal to our bounds analysis and therefore
can only strictly improve Negative Fuzz's error bounds. Penultimately, we
discuss the soundness and semantics of \textbf{factor}. Finally, we discuss
whether our language is missing other related primitives. 

\paragraph{Problem intuition.}
We return to our example pairwise summation program ($(w \tilde{+} x) \tilde{+}
(y \tilde{+} z)$) from Section~\ref{sec:overview} whose type is $M_{3 \cdot u} \
\textbf{num}$:
\begin{equation*}
  \begin{aligned}[c]
...
\caption{
  ...
  occurances of the \textbf{let-bind} typing rule in the typing tree.
}
\end{subfigure}
\caption{
...
corresponding to the iterative summation algorithm on four numbers. 
Note that we are forced to sequence the result of each \textbf{rnd} with the
\textbf{let-bind} typing rule in order to use the result in the remainder of the
computation. 
...
to mirror the typing tree of our iterative summation program (subfigure c). This
...
Observe how the \textbf{factor} primitive allows us to first ``share" the $u$
monadic error grade between two \textbf{add}. Then, only after the error is
shared, we use a single \textbf{let-bind} sequencing operation to pass the
floating-point result to the final \textbf{add}.
Instead of having to use iteratively sequenced \textbf{let-bind}s to
sequentially \textit{add} the grades of multiple monadic computations,
...
We also observe that rewriting a program with \textbf{factor} can only improve
...
\end{enumerate}
Therefore, we find that rewriting a program with \textbf{factor} can only
improve our grade $q$ while keeping the bound $b$ the same. By inspecting our
error theorems (Theorem~\ref{thm:paired-a-priori-abs} and
Theorem~\ref{thm:paired-a-priori-rel}) we note that a program rewritten using
\textbf{factor} to have a lower monadic grade $q$ results in a strictly better
final error bound. 
...
the convention that the left-hand-side of a monadic pair represents the ideal
computation and the right-hand-side of a monadic pair to represent the rounded
computation. This ensures that the monad grade bounds the distance between the
ideal and floating-point computation.
The operational semantics of \textbf{factor} maintains our convention by
reassociating pairs of pairs of values $((v_0, \tilde{v_0}), (v_1, \tilde{v_1}))
\in \mathcal{R}_{M_q \tau \times M_q \tau'}$ to  $((v_0, v_1), (\tilde{v_0},
\tilde{v_1})) = ((v_0, v_1), \widetilde{(v_0, v_1)}) \in \mathcal{R}_{M_q (\tau
\times \tau')}$.
This ensures that \textbf{factor} is sound and preserves metric preservation; if
$v_0$ and $\tilde{v_0}$ and $v_1$ and $\tilde{v_1}$ are both $q$ apart, then
$(v_0, v_1)$ and $(\tilde{v_0}, \tilde{v_1})$ are also $q$ apart.

\paragraph{Other related primitives}
It is natural to wonder if our type system is missing other primitives for
distributing and factoring or monad and comonadic grades. It appears that we are
not.
Our other typing rules can derive the inverse type of \textbf{factor}, namely,
