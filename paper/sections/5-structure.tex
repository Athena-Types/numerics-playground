...
From the numerical analysis literature, we expect the more efficient pairwise summation
algorithm to have type $M_{2 \cdot u} \ \textbf{num}$, a one-third tighter bound
than the iterative summation algorithm (subfigure b).
However, both programs have the same type.
\begin{figure}[htp] 
\centering
...
\DisplayProof
\caption{
  A partial typing tree showing the commonalities of both summation algorithms.
...
The operational semantics of \textbf{factor} respect this convention by stepping
$\textbf{factor} \ ((v_0, \tilde{v_0}), (v_1, \tilde{v_1}))$ to $((v_0, v_1),
(\tilde{v_0}, \tilde{v_1}))$, which viewed as a value of form $((v_0, v_1),
\widetilde{(v_0, v_1)})$.
