...
monadic grades.
We extend Numerical Fuzz's type inference algorithm for our new syntax (e.g.
...
monadic grades.
Type inference then syntactically synthesizes the corresponding well-typed term,
type, and context with bound polymorphism with the appropriate sensitivities and
grade annotations.
Finally, we provide a method to ``simplify" the bound expressions in our type
using the semantic logical relation.
...
\begin{definition}[Sensitivity erasure]
  ...
  \times \ \textit{types}$, 
  a fully erased typing context removes all sensitivity information and only
  returns the type:
...
(e \{ i \})^{\bnd{\bullet}} &\triangleq e^{\bnd{\bullet}}  \\
(\Lambda \epsilon . e)^{\bnd{\bullet}} &\triangleq e^{\bnd{\bullet}} \\
...
Type inference has the following shape: 
$\Gamma^{\bullet}; e^{\bnd{\bullet}} \implies \Gamma'; e'; \tau$.
In other words, we take an erased typing context $\Gamma^{\bullet}$  and an
erased program $e^{\bnd{\bullet}}$ and produce a
typing context $\Gamma$ with sensitivity information and fully annotated program
$e$ and type $\tau$ such that there exists a $\Delta$ such that
$\Delta \ | \ \Gamma' \vdash e' : \tau$
is derivable.
We start type inference by passing the empty context, along with the
bound-erased program (written $e^{\bnd{\bullet}}$) and compute two things:
\begin{enumerate}
  \item We infer co-monadic sensitivities and monadic grades via the type
    inference algorithm provided in \cite{numfuzz}, extended to deal with our
    new $\mathbf{factor}$ primitive. 
...
and bounds are inferred simultaneously.
The Unit, Var, $!$ I, \textbf{op}, Ret, Rnd, $\times$ I, $\times$ E, $\otimes$
I, \textbf{let}, \textbf{let-pair}, \textbf{let-bind}, \textbf{case} cases come
from Numerical Fuzz. 
We outline the remaining cases: $\multimap$ I uses 
$I(\tau^{\bnd{\bullet}})$ to create globally fresh bound variables. $\multimap$
E uses $E(\tau)$ to extract appropriate bound variables and the type
equality check for soundness. \footnote{Both the $I$ and $E$ helper functions are fully
defined in the appendix in Figure~\ref{fig:helper_type_inference}.} The
\textbf{factor} case is new but straightforward.
...
\Gamma^{\bullet}, x : \tau_1 ; f_2^{\bnd{\bullet}} \implies \Theta_1, x :_s \tau_1; f'_2; \tau'
\end{aligned}
$}
...
the appendix.
We do not prove that our type inference algorithm infers the tightest possible
sensitivity or round-off grades.
We only prove that the inferred type, typing contexts, and term, after
...
\paragraph{Simplifying the inferred type.}
...
$. \vdash \textbf{add} \ \langle 1, 2 \rangle : M_u \ \textbf{num}_{\bnd{(1, 1, 0, \text{True}) + (2, 2, 0, \text{True})}}$.
Semantically, this type represents the same relation as:
$. \vdash \textbf{add} \ \langle 1, 2 \rangle : M_u \ \textbf{num}_{\bnd{(3, 3, 0, \text{True})}}$
...
bound expressions. 
For soundness, we prove a semantic property that the logical relation corresponding
to the simplified type is the same logical relation as the original type. 

\begin{definition}[Semantic type simplification]
Our type evaluation scheme $\llbracket - \rrbracket$ takes in a type $\tau$ and
returns a simplified type $\tau'$. For simplicity, we halt simplification when
we see a $\forall$:
\begin{equation*}
  \begin{aligned}[c]
