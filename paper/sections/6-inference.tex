...
analysis developed in Numerical Fuzz. Simultaneously, we automatically infer
polymorphic bounds to generate a valid typing derivation of the program in
\bnd{\Lang} to reduce the user annotation burden of adopting our approach.
To define the type inference problem, we first setup a sensitivity erasure
$-^{\bullet}$ over typing contexts and a bound erasure $-^{\bnd{\bullet}}$ over
types and terms. Type inference then infers sensitivities, grades, and bounds
over erased, well-typed terms in contexts with erased sensitivities.

\begin{definition}[Sensitivity erasure]
  For a fully annotated typing context 
  $\Gamma : \textit{variables} \to \textit{sensitivities} \  \times \ \textit{types}$,
...
detailed in Figure~\ref{fig:inference-algo}. Finally, we evaluate our bound
expressions using the $\llbracket - \rrbracket$ evaluation map defined over
types.
...
\begin{enumerate}
  ...
  \item At the same time as our sensitivity inference, we infer our bounds. We
    ...
    (Figure~\ref{fig:helper_type_inference}) to automatically instantiate and
...
$\llbracket - \rrbracket$ and prove soundness (Theorem~\ref{thm:eval-bounds}).
...
\end{center}
    ...
    in an unerased type. For type checking to succeed on many real-world
    programs, it is important for I and E to align in the production and
    extraction of bound variables. }
...
We use the following lemma for reasoning about the bound polymorphism variables
in the $\multimap I$ and $\multimap E$ cases for the type inference soundness
proof.
...
We use the following lemma for reasoning about the bound polymorphism variables
in the $\multimap I$ case for the type inference soundness proof.
...
We use the following lemma for reasoning about the bound polymorphism variables
in the $\multimap E$ case for the type inference soundness proof.
...
Now we are able to prove the soundness of our type inference algorithm. 
% Note that if the ordering of the variables produced by the I and E helper
% functions do not align, the equality check in the $\multimap$ E case will fail.
% This means that the choice of ordering is not essential for our soundness proof
% but would be relevant for a completeness argument.
Note that we have that $\Gamma^{\bullet} = \Gamma'^{\bullet}$ and
$e^{\bnd{\bullet}} = e'^{\bnd{\bullet}}$ as we do not prove that our type
inference algorithm infers the tightest possible sensitivity or round-off
grades.
...
\begin{description}
  ...
  \item[Var.] Let $\Delta$ contain all the bound variables mentioned by $\Gamma$
    and $\tau$. Then we trivially have a typing derivation for $\bnd{\Delta \ |
  ...
  \item[$\multimap$ I.] By our inductive hypothesis, we know that there exists a
    ...
    must satisfy, from the premisses, that:
  ...
  % TODO: spell out delta
    ...
    By weakening on our inductive hypothesis, there exists a $\Delta$ such that
    we have a derivation for $\Delta \ | \ \Theta \vdash e : \tau_0[\epsilon_0 /
    i_0, \ldots, \epsilon_n / i_n]$.
  ...
  \item[$\otimes$ E.] We finish by applying our $\otimes$ E typing rule.
\end{description}
\end{proof}
...
(Definition~\ref{def:bound-exp-eval}) evaluating bound expressions to also
...
evaluated type should be the same logical relation as the original type. This is
...
\begin{proof}
  ...
  The structural cases are immediate. The only non-structural case evaluates
  $\textbf{num}_{\bnd{b}}$ to $\textbf{num}_{\llbracket \bnd{b} \rrbracket}$.
  Since $\mathcal{R}_{\bnd{b}} = \mathcal{R}_{\bnd{\llbracket b \rrbracket}}$
  refer to the same underlying set, we are done.
\end{proof}
