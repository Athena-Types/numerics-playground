...
\begin{enumerate}
  ...
  \item At the same time as our sensitivity inference, we infer our bounds. We
    ...
    (Figure~\ref{fig:helper_type_inference}) to discover which polymorphic bound
    variables needed to be automatically instantiated and eliminated.
\end{enumerate}
...
instantiated. Both the $I$ and $E$ helper functions are fully defined in Figure
~\ref{fig:helper_type_inference}. Finally, the \textbf{factor} case is
straightforward.
...
\paragraph{Type inference soundness.}
We now prove the soundness of our type inference algorithm.
...
So, our soundness theorem states that the inferred type, typing contexts, and
term, after erasing, are equal to the original erased type, erased context, and
erased term ($\Gamma^{\bullet} = \Gamma'^{\bullet}$ and $e^{\bnd{\bullet}} =
e'^{\bnd{\bullet}}$).
