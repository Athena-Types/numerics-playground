...
We extend the subtyping relation defined in Numerical Fuzz for type inference.
In particular, we add the following cases to deal with bounds and bound
polymorphism.
$$
\begin{aligned}[c]
...
The proof of the admissibility of subtyping is similar to the proof given in
Numerical Fuzz \cite{numfuzz}.
...
\end{theorem}
\begin{proof}
  We induct over the derivation $\Delta \ | \ \Gamma \vdash e : \tau$.
% TODO: inline NumFuzz proof
  Many cases are immediate.
  Some require weakening, which we proved in (Theorem \ref{thm:weakening}).
...
% TODO: define supertyping, needed for ty inf
