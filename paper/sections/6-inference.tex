...
sensitivities and types with possibly-polymorphic bounds. This can be cumbersome to
manually annotate.
...
\subsection{Evaluating bound expressions}
...
$. \vdash add \ \langle 1, 2 \rangle : M_u \ \textbf{num}_{(1, 1, 0, \text{True}) + (2, 2, 0, \text{True})}$
Semantically, this type represents the same relation as:
$. \vdash add \ \langle 1, 2 \rangle : M_u \ \textbf{num}_{(3, 3, 0, \text{True})}$

In this case, we can extend the $\bnd{\llbracket - \rrbracket}$
(Definition~\ref{def:interval-exp-eval}) evaluating bound expressions to
also evaluate types that possibly contain bound expressions. 
Importantly our soundness theorem proved below, the logical relation
corresponding to the evalauted type should be the same logical relation as the
original type. This is a semantic notion of soundness, which is why we present
the evaluation of types and bound expressions separately from our
syntax-directed type inference algorithm.
