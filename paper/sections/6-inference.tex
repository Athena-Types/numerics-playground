...
Full typing derivations can be seen as maps from variables to pairs of
sensitivities and types with interval bounds. This can be cumbersome to manually
annotate.

Therefore, we introduce a type inference algorithm that allows users to write
programs in \Lang, without sensitivities or bound polymorphism. We compute the
type sensitivity and inference analysis developed in \cite{numfuzz}, extended to
our core language without bound polymorphism.

At the same time, we also automatically infer polymorphic bound annotations to
generate a valid typing derivation of the program in \bnd{\Lang}. To define the
type inference problem, we first define a sensitivity erasure $-^{bullet}$ over
typing contexts and a bound erasure $-^{\bnd{\bullet}}$ over types and terms. We
will then define type inference as inferring sensitivities and bounds over
erased, well-typed terms.
...
% TODO: missing cases (otherwise is not correct)
\end{aligned}
\end{equation}
\end{definition}
...
Finally, we evaluate our bound expressions using the $\llbracket - \rrbracket$
evaluation map defined over types.
...
\begin{enumerate}
  ...
  \item Finally, we compute our interval operations via $\llbracket - \rrbracket$. The output of bound inference may
    produce interval expressions that can be further reduced.
\end{enumerate}
We present our type inference algorithm in Figure~\ref{fig:inference-algo} as a
combined algorithm where the sensitivities and bounds are inferred
...
inference algorithm. 
After running type inference, we evaluate the closed interval bound expressions
using $\llbracket - \rrbracket$ and prove soundness via a semantic logical
relations argument.
...
\subsection{Evaluating interval bounds}
After running type inference we obtain a result in the form: 
$\Delta \ | \ \Gamma \vdash e : \tau$. 
In some cases, $\tau$ might contain interval expressions.
In this case, we can extend the $\bnd{\llbracket - \rrbracket}$
(Definition~\ref{def:interval-exp-eval}) evaluating intervals expressions to
also evaluate types that possibly contain interval expressions. Note that for
simplicity, we only halt evaluation when we see a $\forall$.
...
\begin{theorem}[Soundness of evaluating closed interval bounds]
