...
\end{equation*}
    ...
    \vskip 0.3em
    \AXC{$\frac{1}{s} \cdot \sigma_0 \Vdash e : \tau' \rightsquigarrow^*
    (v_0^0, v_1^0);(v_0^1, v_1^1);...; (v_0^n, v_1^n) : \tau' \tensor \tau''$}
    \UIC{ $\sigma_0 + \sigma_1 \Vdash \textbf{let-pair}_{(s, \tau', \tau'')} \
    (x, y) = e \ \tin \ f  : \tau \rightsquigarrow \linebreak \sigma_1[x \mapsto
    v_0^0 :_s \tau', y \mapsto v_1^0 :_s \tau'']; \sigma_1^{-1}[x \mapsto v_0^1
    :_s \tau', y \mapsto v_1^1 :_s \tau'']; ...; \sigma_1^{-1}[x \mapsto v_0^n
    :_s \tau', y \mapsto v_1^n :_s \tau''] \Vdash f : \tau$}
    \DisplayProof
