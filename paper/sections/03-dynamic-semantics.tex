...
\begin{equation*}
  \begin{aligned}[c]
    ...
    % factor
    % (\sigma_0; \sigma_1);(\sigma_2; \sigma_3) \Vdash \factor v \ :
    % (M_q~\tau_1 \times M_r~\tau_2) &\rightsquigarrow (\sigma_0; \sigma_2);(\sigma_1;
    % \sigma_3) \Vdash v : M_{max(q, r)} (\tau_1 \times \tau_2) \\
  \end{aligned}
\end{equation*}
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Structural rule
    ...
    % factor rule
    \vskip 0.3em
    \AXC{$ \sigma \Vdash v \ : (M_q~\tau_1 \times M_r~\tau_2) \rightsquigarrow
    (\sigma_0; \sigma_1);(\sigma_2; \sigma_3) \Vdash v : (M_q~\tau_1 \times
    M_r~\tau_2)$}
    \UIC{$\sigma \Vdash \textbf{factor}~e : M_{max(q, r)} (\tau_1 \times
    \tau_2) \rightsquigarrow (\sigma_0; \sigma_2);(\sigma_1; \sigma_3) \Vdash v
    : M_{max(q, r)} (\tau_1 \times \tau_2)$}
    \DisplayProof
    % let rule
    \vskip 0.3em
    \AXC{$\sigma_1 \Vdash e : \tau' \rightsquigarrow^* \sigma_1' \Vdash v_1 :
    \tau'$}
    \AXC{$\sigma_1' @ \sigma_0 [x \mapsto v_1 :_s \tau'] \Vdash f
    \rightsquigarrow^* \sigma_2 \Vdash v_2 : \tau$} 
    \BIC{ $\sigma_0 + s \cdot \sigma_1 \Vdash \textbf{let}_{(s, \tau')} \ x = e
    \ \tin \ f  : \tau \rightsquigarrow \sigma_2 \Vdash v_2 : \tau$}
    \DisplayProof
    % let-bind rule
    \vskip 0.3em
