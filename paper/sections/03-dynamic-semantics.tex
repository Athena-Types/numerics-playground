...
term in an enviroment to a typed term in an enviroment.
...
Useful theorem. If $\sigma \Vdash e : \tau \rightsquigarrow \sigma' \Vdash e' :
\tau'$, then $\llbracket \sigma \Vdash e : \tau \rrbracket = \llbracket \sigma
...
% this in prior work.
% Max [May 1st, 1 AM]: I still need to figure out what $\llbracket \sigma
% \Vdash e : \tau \rrbracket$ means. One option is to just erase the values and
% interpret it the same way as in the original NumFuzz paper (from metric spaces
% to metric spaces). This probably works, but there might be another way.
