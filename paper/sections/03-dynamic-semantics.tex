...
\begin{equation*}
  \begin{aligned}[c]
    ...
    \Vdash f : M_q \tau \\
    % let-cobind rule
    \sigma \Vdash \textbf{let-cobind}_{(s, t)} \ x = v \ : \tau \ \tin \ f &\rightsquigarrow (\sigma[x
    \mapsto v :_s \tau_1]) * t
    \Vdash f : \tau \\
    % lam app
...
\end{center}
    ...
    information (tracked with metavar $s$), error (tracked with metavars $r,
    q$), and type of bound variables $\tau_1, \tau_2$, is preserved as
    annotations in the syntax, written $[e]_s$ and $\textbf{let-bind}_{(s,
    \tau_1)}$, $\textbf{let-cobind}_{(s, t)}$, and $\lambda x : \tau_1 . e $.}
    \label{fig:sub_eval_rules}
\end{figure}
