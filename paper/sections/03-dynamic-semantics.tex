...
\end{equation*}
    ...
    \sigma'[x \mapsto v_1 :_s \tau']; ...; \sigma'[x \mapsto v_n :_s \tau']
    \Vdash f : M_q \tau$}
    \DisplayProof

    % let-cobind rule
    % \sigma \Vdash \textbf{let-cobind}_{(s, t, \tau')} \ x = v \ : \tau \ \tin \ f &\rightsquigarrow (\sigma[x
    % \mapsto v :_s \tau']) * t
    % \Vdash f : \tau \\
    % [e] rule
    ...
    \vskip 0.3em
    \AXC{$\sigma \Vdash e : \tau' \rightsquigarrow \sigma' \Vdash e' : \tau_2$}
    \UIC{$\sigma \Vdash \textbf{let}^* \ x = e \ : \tau' \ \tin \ \tau_2
    \rightsquigarrow \sigma' \Vdash \textbf{let}^* \ x = e' \ : \tau' \
    \tin \ \tau_2$}
    \DisplayProof
