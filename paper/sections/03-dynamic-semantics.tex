...
$$
where $\sigma =  \gamma_0;...;\gamma_k$. Note that for each program
configuration, either the notation on the left or on the right will be used;
they are not be mixed.
...
\end{equation*}
    ...
    \gamma_0 \Vdash v_0;...; \gamma_n \Vdash v_n : M_r \tau'$} \UIC{ $\sigma_0 +
    \sigma_1 \Vdash \textbf{let-bind}_{(s, \tau')} \ x = e \ \tin \ f  : M_{q}
    \tau \rightsquigarrow \gamma_n, \sigma_0 [x \mapsto v_n :_s \tau'];
    \gamma_0, \sigma_0 [x \mapsto v_0 :_s \tau'] \Vdash f : M_q \tau$}
    \DisplayProof
    ...
    % sharing rule 
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % TODO: update rule to thread \sigmas
    \vskip 0.3em
