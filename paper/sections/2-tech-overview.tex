...
One way to view this is to imagine two large nearby numbers with small relative
error. Subtracting the two numbers could obtain a small number with arbitrarily
high relative error.
...
original unpaired representation reduces the precision of our analysis on
...
containing only-growing functions on non-negative numbers will be non-negative.
When our bounds analysis successfully proves that a result is non-negative, the
terms in our main error theorem reliant on the bounds analysis disappear that
the maximum round-off error is simply the grade of the monad.
