...
$\textsc{binary32}$ and $\textsc{binary64}$ which fix $p = 24$ and $p = 53$
respectively.
...
We consider the well-known pairwise summation algorithm (sometimes referred to
as cascade summation).
As a concrete running example, we consider adding up four floating-point numbers
$w, x, y, z \in [-1, 1]$ via pairwise summation: $\rho(\rho((w + x)) + \rho((y +
z)))$.
...
in their inputs could obtain a small number with arbitrarily high relative
round-off error.
...
% TODO: more intuition if possible
...
Numerical Fuzz's type system relies on bounding function sensitivity with
respect to Olver's relative precision metric. 
...
$M_{\infty}~\textbf{num}$
in the context 
$w : \textbf{num}, x : \textbf{num}, y : \textbf{num}, z :
\textbf{num}$.\footnote{%
  For presentational purposes, we elide the sensitivity co-effect annotation in
the typing context. The program is infinitely-sensitive in variables $w,x,y,z$.
}
...
Therefore, we can only show that our example program has infinitely-bounded
error, which is not a useful error bound.

\paragraph{Paired model.}
As addition and subtraction over the unrestricted reals are infinitely-sensitive
with respect to Olver's relative precision metric, we first deal with the
problem of infinite sensitivity.
We develop a paired model for arithmetic where unrestricted addition and
subtraction have finitely-bounded sensitivity with respect to relative
precision. The trick is to associate for each real $r \in \mathbb{R}$ a triple
$(r, a, b) \in \mathbb{P} = \mathbb{R}
...
problem of catastrophic cancellation in the paired model.
...
By ensuring that each operation in the paired model is always-growing,
we can encode addition and subtraction in a finitely-sensitive manner. 

In the paired model, we can implement and type addition and
...
have the same type as in Numerical Fuzz. Subtraction has the same type as
...
\paragraph{Using the type to assign a floating-point error bound.}
Now we wish to translate our type over the paired model into an error bound over
the unpaired model.
...
representation of $w, x, y, z \in [-1, 1] \times [0, 1] \times [0, 1]$. 
Our bounds analysis will take this input and determine that the ideal
computation has type $\textbf{num}_{\bnd{((-4, 4), (0, 4), (0, 4))}}$, ensuring
that the program result will be within $[-4, 4] \times [0, 4] \times [0,
...
To obtain (2), we can deduce from the $3u$ grade of our monad that for all
inputs $w, x, y, z$ to our input program, for an output value $(r, a, b),
(\tilde{r}, \tilde{a}, \tilde{b})$, that $d(a, \tilde{a}) \leq 3u$ and $d(b,
\tilde{b}) \leq 3 u$. 
...
% TODO: can I go even slower here and explain things better?
\paragraph{Bound polymorphism.}
It is challenging to assign a single, most precise type to $\textbf{add}$.
The difficulty arises when $\textbf{add}$ is called multiple times in
different contexts.
Looking at the first two add calls, we could try to assign the type: 
$\textbf{add} : \textbf{num}_{\bnd{((-1, 1), (0, 1), (0, 1))}} \times
\textbf{num}_{\bnd{((-1, 1), (0, 1), (0, 1))}} \multimap
\textbf{num}_{\bnd{((-2, 2), (0, 2), (0, 2))}}$.
This would make it impossible to type the third add call.
However, if we look at the third add call, we could try to assign the
type: 
$\textbf{add} : \textbf{num}_{\bnd{((-2, 2), (0, 2), (0, 2))}} \times
\textbf{num}_{\bnd{((-2, 2), (0, 2), (0, 2))}} \multimap
\textbf{num}_{\bnd{((-4, 4), (0, 4), (0, 4))}}$
But now our analysis is imprecise in our first two calls to
$\textbf{add}$.

To ensure that our bounds analysis is scalable and that function types can be
...
We also introduce \textit{bound operations} (e.g. $+$, $-$, and $\cdot$), which are
meta-level operations mapping bounds to bounds that enable us to type:
$$
...
$$
To reduce the user annotation burden, we provide a type inference algorithm that
can automatically infer bound polymorphism abstraction and instantiation sites.
We prove our type inference algorithm sound in Section~\ref{sec:inference} and
...
us to obtain the expected $O(log_2(n))$ bound for pairwise summation.
