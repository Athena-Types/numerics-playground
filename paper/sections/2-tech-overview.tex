...
example. Then, we overview our extensions.
...
floating-point system as the exact operation followed by rounding by a rounding
function (written $\rho$) to a float.
...
standard rounding error model assumes $\rho$ will observe round-off error
...
We wish to support a error analysis on programs containing subtraction
or negative numbers. 
Unfortunately, subtraction in Numerical Fuzz is infinitely sensitive. 
...
If we perturb the input by a small amount, for example, by 1 per-cent, we can
...
in its inputs could obtain a small number with arbitrarily high round-off error,
as measured using relative precision.
...
To understand the problem posed by an infinitely-sensitive operation, consider
our running example:
\begin{equation}
\begin{aligned}[c]
...
Our type system relies on bounding function sensitivity. 
...
Under the paired representation, we can implement and faithfully type addition,
subtraction, and multiplication in terms of finitely-sensitive operations over
the paired components. Addition is defined below:
\begin{equation}
  \begin{aligned}[c]
...
Now we wish to translate these bounds into something useful over the unpaired
component.
...
In our running example, we have that the paired representation of 
...
Further, we can deduce from the grade of our monad that for all inputs
...
$\rho(\rho(\rho(w~+~x)~+~y)~+~z)$.
In other words, the error bound obtained by Numerical Fuzz grows linearly in
...
term language inspired by linear logic. Rewriting our running example with our
special primitive below, we can see that using factor results in a significantly
smaller error grade for a program representing the same association and order of
operations. 
A smaller monadic error grade results in a smaller floating-point error bound,
reducing the error of our running example by approximately one-third from
$2.6645352591003757 \times 10^{-15}$ to $1.7763568394002505 \times 10^{-15}$.
