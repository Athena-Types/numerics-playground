...
There is some difficulty in assigning a single, most precise type to
$\textbf{add}$.
...
example grows in $O(log_2(n))$. However, Numerical Fuzz will infer that the
error on our paired summation example grows in $O(n)$.
The issue is that the monadic computation
sequencing rule forces the error analysis to always compute the worst-case
round-off error associated with the most error-prone addition summation
$\rho(\rho(\rho(w~+~x)~+~y)~+~z)$, which yields an undesirable,
overly-conservative error analysis.
...
We accomplish this improvement by developing a special primitive
$\textbf{factor}$ in our term language inspired. Rewriting our running example
with our special primitive below, we can see that using factor results in a
significantly smaller error grade for a program representing the same
association and order of operations. 
