...
rounding function $\rho$, \textbf{add} to represent exact additiion ($+$) and
...
Now we wish to translate these bounds over the paired components into a error
bound over the unpaired component.
...
summation tree. 
Returning to our running example, pairwise summation will recursively a list of
numbers in half and compute the sum of the halves.
This produces a computation tree of height $\left \lceil{log_2(n)}\right \rceil$
with error growing in $O(log(n))$.
