...
sensitivity: $[e]$). 
We have several \textbf{let-bind} and \textbf{let-cobind} for sequencing and
structuring monadic and comonadic computation respecitvely. We have \textbf{case
\ldots of \ldots} for conditional reasoning on sum types.

We introduce a new language for bounds (shown in Figure~\ref{fig:syntax}) as
...
simultaneously track round-off error and function sensitivity. 
...
\begin{definition}[Context scaling]
  ...
  \cdot \Gamma'(x)$
\end{definition}
Bound contexts have the grammar $\Delta ::= . \ | \ \Delta, b : \textbf{bnd}$
...
We use our typing context and judgement to indicate that the variable $x$ is
$s$-sensitive in the expression $e$ and has worst-case round-off error
upper-bounded by $q$.
\begin{equation}
...
corresponding to the unit round-off constant. Note that $M_u E$ is the
...
polymorphism. We have a \bnd{inj} function for mapping constants $k$ into the
...
Note that we only define our language over closed terms. We also define
...
\begin{figure}
  ...
  \end{alignat*}
  \caption{Grammer for evaluation contexts.}
...
\begin{description}
  ...
  \item[c) Property of $\mathbf{op} \in \Sigma$: metric preservation.] 
    The bolded \textbf{op} is our syntacic metavariable for our supported
    operations (e.g. \textbf{add}, \textbf{sub}, \textbf{mul}).
    The italizied $\textit{op}$ is the metavariable represents the corresponding
    function mapping values to values.
    We require that \textit{op} is 1-sensitive as measured by $\mathcal{SD}$.
    To simplify our presentation, we also assume that for every operation has the following
...
\begin{lemma}[$\mathcal{SD}$ is preserved under stepping]
  ...
  \mathcal{SD}(e'_0, e_1)$. Note that by metric symmetry, $\mathcal{SD}$ is
  preserved under stepping on both sides.
\end{lemma}
\begin{proof}
...
\begin{proof}
  ...
  \textbf{$\otimes$ I} rules, the enviroment must be split into a $\Gamma$
...
$r$-sensitive substitution lemma. Note that the $r=0$ restriction helps the
$\otimes E$ case to go through.
...
\begin{proof}
  ...
  holds.
\end{proof}
Finally, we can state our main error soundness theorem, which we prove in the
appendix.
