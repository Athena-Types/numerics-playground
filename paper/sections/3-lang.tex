...
\AXC{$\ldots$}
\AXC{$\Delta \bdash b_c : \textbf{bnd}$}
\AXC{$\textbf{iop}_c \in \Sigma_{\textbf{bnd}}$}
\RightLabel{(\bnd{\text{Bop}})}
\QIC{$\Delta \bdash \textbf{iop}_c(i_0, \ldots, i_c) : \textbf{bnd}$}
\bottomAlignProof
\DisplayProof
\vskip 1em
...
\subsection{The lattice of interval expressions}
For a given lattice $(\textit{num}, \leq)$ we build up to the definition of
lattice over interval expression equivalence classes. 
First we define \textit{concrete intervals}, which will serve as our canonical
representative for each equivalence class:

\begin{definition}[Concrete intervals and lattice]
  The set of concrete intervals $C$ are
  $C \triangleq (k_0, k_1) \in \textit{num}^2$ where each 
  $\bnd{\llbracket} (k_0, k_1) \bnd{\rrbracket} = 
  \{ k \ | \ \forall k \in \textit{num}, \ k_0 \leq k \leq k_1 \}$ 
  equipped with the standard $\subseteq$ relation over sets.
  Then, $(C, \subseteq)$ forms our concrete interval lattice.
\end{definition}

Each interval operation $\textbf{iop}$ of arity $n$ has a corresponding mathematical
function $\textit{iop} : \textit{num}^n \to \textit{num}$. 
We can now define our equivalence classes of closed \textit{interval expressions},
which we will use to define our logical relations over.

\begin{definition}[Closed interval expressions]
  An interval expression $b$ is a closed interval expression iff either:
  \begin{itemize}
    \item $b = (k_0, k_1)$; it's a constant
    \item $b = \textbf{iop}(b_0, \ldots, b_1)$ where $b_0, \ldots, b_1$ are all
      closed interval expressions
  \end{itemize}
\end{definition}

\begin{definition}[Evaluation of closed interval
  expressions] \label{def:interval-exp-eval}
  For an interval expression $b$,
  \begin{equation}
    \begin{aligned}[c]
      \bnd{\llbracket} (k_0, k_1) \bnd{\rrbracket} &\triangleq \{ k \ | \ \forall k \in \textit{num}, \ k_0 \leq k \leq k_1 \} \\
      \bnd{\llbracket} \textbf{iop}(b_0, \ldots, b_n) \bnd{\rrbracket} &\triangleq \textit{iop}(b_0, \ldots, b_n)
    \end{aligned}
  \end{equation}
\end{definition}

\begin{definition}[Interval expression equivalence class]
  We inductively define our equivalence relation $\sim$ over closed interval
  expressions $I$ based upon the underlying equality of the sets
  $b_0 \sim b_1 \iff \bnd{\llbracket} b_0 \bnd{\rrbracket} =  \bnd{\llbracket} b_0 \bnd{\rrbracket}$ 
\end{definition}

\begin{lemma}
  $I, \sim$ form an equivalence class
\end{lemma}
\begin{proof}
...
formed by our interval equivalence class $I / \sim$. We will later use this to
define our logical relation over these interval bound equivalence classes.

\begin{definition}[Interval expression lattice]
...
\begin{definition}[Distance between closed syntactic terms]
  ...
  \begin{aligned}[c]
    ...
    \mathcal{SDV}_{\mathbf{num_{b}}}(k_0, k_1) &\triangleq 
      d_\mathbf{num} &\text{ for } k_0, k_1 \in \llbracket b \rrbracket \\
    ...
    \mathcal{SDV}_{\textbf{bnd}}((k_0, k_1), (k'_0, k'_1)) 
      &\triangleq \infty &\text{ for } k_0 \not= k'_0, k_1 \not= k'_1 
    \\
    \mathcal{SDV}_{\forall \epsilon . \tau}(v_0, v_1) 
      &\triangleq \text{sup}_{w \in \textit{num}} 
      \mathcal{SD}_{\tau[w / \epsilon]}(v_0~\{w\},~v_1~\{w\})
    \\
...
\begin{definition}[Interface for instantiating Numerical Fuzz.]
  ...
  $\leq_{\textit{num}}$), $d_\textbf{num}$, $\rho$, $u$, $\Sigma$, and
  $\Sigma_{\bnd{bnd}}$ such that the following properties hold: 
\begin{description}
  \item[a) The lattice (\textit{num}, $\leq$) and distance function $d_\textbf{num}$ form a
  metric space.]
  ...
  \item[c) Property of $\mathbf{iop} \in \Sigma_{\bnd{bnd}}$: closure over intervals ($\textit{num}^2$).] We further need to
    assume that for every operation $\mathbf{iop}(b_0, \ldots, b_n) : \Sigma_\textbf{num}$, we
    have that if $b_0, \ldots, b_n$ are closed interval expressions for any sequence of bounds $b_0, \ldots, b_n : \textbf{bnd}$ the
    ...
    holds.
  \item[d) Property of $\mathbf{op} \in \Sigma$: metric preservation.] 
  ...
  keep the interface minimal for presentational purposes.}
    ...
    v$ for all $\bnd{i_0}, \bnd{i_1} \in \mathbb{\textit{num}}^2$ 
    and for all v mapping bounds and syntactic values in
    $(\tau_0 \to \tau_1)[i_0 / \epsilon_0, i_1 / \epsilon_1]$
    is 1-sensitive as measured by $\mathcal{SD}$.
...
\begin{definition}[Logical relation]
  ...
  \begin{aligned}[c]
    ...
    \mathcal{VR}_{\mathbf{num}_{b}} &\triangleq \llbracket b \rrbracket \\
    ...
    \mathcal{VR_{\forall \epsilon. \tau}} &\triangleq 
      \{ \Lambda \epsilon . v \ | \ \forall (k_0, k_1) \in \textit{num}^2, \ v ~ 
        \{(k_0, k_1)\} \in
...
\begin{lemma}[Admissibility of weakening (on bound contexts)]\label{thm:weakening-bnd}
  If $\Delta \bdash i : \textbf{bnd}$ and $\Delta \subseteq \Delta'$, then
  $\Delta' \bdash i : \textbf{bnd}$. 
\end{lemma}
\begin{proof}
