...
\QIC{$\Delta \bdash \textbf{bop}(b_0, \ldots, b_n) : \textbf{bnd}$}
\bottomAlignProof
\DisplayProof
\vskip 1em
...
Our rewrite relation has a restricted ability to step bound expressions and
...
\begin{align*}
  ...
  \textbf{bop}_c(c_0, \ldots, c_n) &\mapsto \textit{bop}_c(c_0, \ldots, c_n)
  \\
...
\subsection{The partial order of bounds} \label{sec:bexp}
For a given partial order $(\bnd{\mathbb{B}}, \leq)$ we build up to the definition of
lattice over bound equivalence classes. 
First we call $\bnd{\mathbb{B}}$ our \textit{concrete bounds}, which will serve as our
...
\bnd{\mathbb{B}}$. 
We can now define our equivalence classes of closed \textit{bound expressions},
...
\begin{definition}[Closed bound expressions]
  ...
  \begin{itemize}
    \item $b = c \in \mathbb{B}$; it's a constant
    \item $b = \bnd{\textbf{bop}}(b_0, \ldots, b_1)$ where $b_0, \ldots, b_1$ are all
      closed bound expressions; it's made up of constants
  \end{itemize}
\end{definition}

\begin{definition}[Evaluation of closed bound
  expressions] \label{def:bound-exp-eval}
...
\begin{definition}[Bound expression equivalence class]
  ...
  expressions $B$ based upon the underlying equality of the sets
  $b_0 \sim b_1 \iff \bnd{\llbracket} b_0 \bnd{\rrbracket} = \bnd{\llbracket} b_0 \bnd{\rrbracket}$ 
\end{definition}

\begin{lemma}
  $B, \sim$ form an equivalence class
\end{lemma}
\begin{proof}
...
formed by our interval equivalence class $B / \sim$. We will later use this to
...
between bounds, expressions, vectors of expressions, and substitutions. This
...
\begin{definition}[Interface for instantiating Numerical Fuzz.]
  ...
  $(\mathbb{B}, \leq)$, $d_\textbf{num}$, $\rho$, $u$, $\Sigma$, and
...
\begin{description}
  ...
  \item[c) Partial order.] $(\mathbb{B}, \leq)$ form a partial order.
  \item[d) Property of $\mathbf{bop} \in \Sigma_{\bnd{bnd}}$: closure over bounds ($\textit{num}^2$).] 
...
\begin{proof}
  ...
  bounds. Therefore for the following cases, we omit reptitively reasoning
