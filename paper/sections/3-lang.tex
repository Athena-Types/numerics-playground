...
scale typing contexts. We represent context summing as $\Gamma + \Delta$ and
...
and can be viewed as a set whose membership indicates whether the variable is in
...
\begin{definition}[Closed bound expressions]
  ...
  \begin{itemize}
    ...
    \item $b = \bnd{\textbf{bop}}(b_0, \ldots, b_n)$ where $b_0, \ldots, b_n$
...
\begin{definition}[Distance between closed syntactic terms]
  ...
  \begin{aligned}[c]
    ...
    \mathcal{SDV}_{\tau_0 \times \tau_1}((v_0, v_1), (w_0, w_1)) 
      &\triangleq max(\mathcal{SDV}_{\tau_0}(v_0, w_0),~\mathcal{SDV}_{\tau_1}(v_1, w_1))
    \\
    \mathcal{SDV}_{\tau_0 \otimes \tau_1}((v_0, v_1), (w_0, w_1)) 
      &\triangleq \mathcal{SDV}_{\tau_0}(v_0, w_0) + \mathcal{SDV}_{\tau_1}(v_1, w_1)
    \\
    ...
    \mathcal{SDV}_{\forall \epsilon . \tau}(v_0, v_1) 
      ...
      \mathcal{SD}_{\tau[c / \epsilon]}(v_0~\{c\},~v_1~\{c\})
    \\
...
\begin{description}
  ...
  \item[c) Property of $\mathbf{op} \in \Sigma$: metric preservation.] 
    ...
    The italicized $\textit{op}$ is a metavariable representing the corresponding
    ...
    $\textbf{op} : \forall \epsilon_0, \epsilon_1 . \tau_0 \multimap \tau_1 \in
    \ \Sigma$.\footnote{%
      Note that we could make this portion of the interface more general
