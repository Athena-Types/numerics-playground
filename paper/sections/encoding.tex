...
such that $r = a - b$. We equip our set with 
a distance functionn $d_{\mathbb{P}}((r, a, b)(\tilde{r}, \tilde{a}, \tilde{b}))$
...
\begin{theorem}[Paired error theorem; a posteriori relative] \label{thm:paired-a-posteriori-rel}
  ...
  to all of the following bounds if $0 < r'$: 
  \begin{enumerate}
  ...
  \end{enumerate}
  and the following bounds if $r' < 0$:
  \begin{enumerate}
    \item[(c)] $max(|ln(e^{-q} + \frac{a^{\uparrow}}{r'}(1-e^{-2q}))|, |ln(e^{q} + \frac{a^{\downarrow}}{r'}(1-e^{2q}))|)$
    \item[(d)] $max(|ln(e^{q} + \frac{b^{\uparrow}}{r'}(e^{2q}- 1))|,|ln(e^{-q} + \frac{b^{\downarrow}}{r'}(e^{-2q}-1))|)$
  \end{enumerate}
\end{theorem} 
\begin{proof}
  ...
  We now examine the scenario where $r' < 0$. This scenario largely mirrors the
  first one, except some directions are flipped.
  Similarly to the previous theorem, there are two cases, $r \leq r'$ or $r >
  r'$. In the first case, we wish to minimize $a-b$ and maximize $a' - b'$. So, 
  \begin{equation}
  \begin{aligned}[c]
