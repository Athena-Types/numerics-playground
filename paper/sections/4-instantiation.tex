...
to read a type as a floating-point error bound.
...
theorem over the unpaired real. We defer introducing our approach to automatic
...
interface defined in Definition~\ref{def:numfuzz-interface} and show that it
satisfies the properties specified. In the subsequent subsections, we
instantiate the interface with the appropriate rounding functions and round-off
parameters
...
Fuzz for eachby floating-point format and rounding mode shown in
...
compute and are typed over our paired representation. Addition is defined as
addition over each of the components in the pair. Subtraction is defined by
swapping the paired components. Multiplication is defined by distributing each
...
condition holds, the our interval bound for multiplication drastically tightens.
...
% case
We set our bound domain $\bnd{\mathbb{B}} = (\mathbb{R} \times \mathbb{R})
\times (\mathbb{R}^+ \times \mathbb{R}^+) \times (\mathbb{R}^+ \times
\mathbb{R}^+) \times \textbf{Bool}$ such that each element of
$\bnd{\mathbb{B}}$, written
$((r^{\downarrow}, r^{\uparrow}), (a^{\downarrow}, a^{\uparrow}),
(b^{\downarrow}, b^{\uparrow}), d)$ represents a set of tuples 
$(r, a, b) \in \mathbb{P}$
satisfiying the following constraints:
\begin{enumerate}
  \item Lower and upper bounds on $r$, with the following constraints $r^{\downarrow} \leq r \leq r^{\uparrow}$
  \item Lower and upper bounds on $a$, with the following constraints $a^{\downarrow} \leq a \leq a^{\uparrow}$
  \item Lower and upper bounds on $b$, with the following constraints $b^{\downarrow} \leq b \leq b^{\uparrow}$
...
whether the boolean condition is true for both input numbers. This dramatically
improves the precision of our analysis; on one of our large-scale benchmarks, we
saw that this minor change tightened the final error bound from $1.11 \times
10^{139}$ to $5.66 \times 10^{-13}$.
\begin{equation}
  \begin{aligned}[c]
    \begin{aligned}[c]
...
We prove in the appendix that our instantation ($\mathbb{P}, d_{\mathbb{P}},
...
\begin{theorem}[Paired error theorem; absolute] \label{thm:paired-a-priori-abs}
  ...
  \begin{enumerate}
    ...
    \item $b \in [b^{\downarrow}, b^{\uparrow}]$
  \end{enumerate}
  we have that $|r - r'|$ is less than or eqaul to the maximum of the following:
\begin{enumerate}
...
\begin{corollary}
  ...
  less than or eqaul to the maximum of the following:
\begin{enumerate}
