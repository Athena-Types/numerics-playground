...
model}$. In particular, we define and type our supported operations
...
theorem over the unpaired real. 
...
that our instantiation adheres to the specification demanded by Negative Fuzz's
...
\begin{tabular}{ c c c c }
 \hline
 \textbf{Parameter}& \textbf{\textsc{binary32}} & \textbf{\textsc{binary64}} &\textbf{\textsc{binary128}} \\
 \hline
 $p$   & 24    & 53 & 113     \\
 \hline
\end{tabular}
...
\begin{table}
  \caption{Common Rounding Functions (modes). $\mathbb{F}$ represents the set of
  floats for a floating-point number set.}
\label{tab:rnd_modes}
\begin{tabular}{ c c c c }
 \hline
\textbf{Rounding mode} & \textbf{Behavior} & \textbf{Unit Roundoff ($u$)} \\
\hline
 Round towards $+\infty$   & $\min\{y \in \F \mid y \ge x \}$  & $2^{1-p}$   \\
 Round towards $-\infty$   & $\max\{y \in \F \mid y \le x \}$  & $2^{1-p}$ \\
 Round towards $0$   & round towards $+\infty$ if $x < 0 $, otherwise $-\infty$ &
 $2^{1-p} $ \\
 Round towards nearest\footnote{%
   For round towards nearest, there 
...
components.
Importantly, each operation maintains the invariant of the paired representation
that $r=a-b$.
\begin{equation}
\begin{aligned}[c]
...
operation is ensured by the interface requirement that each primitive
...
Theorem~\ref{thm:paired-a-priori-abs}), which for a triple $(r, a, b)$
