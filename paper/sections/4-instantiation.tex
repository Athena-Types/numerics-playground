...
In this section we instantiate Numerical Fuzz to provide forwards error bounds
...
such that $r = a - b$. We equip our set $\mathbb{P}$ with a distance function
$d_{\textit{num}}((r, a, b)(r', a', b'))$ forming a metric
space.
To soundly use our encoding, we must instantiate the modular interface defined
in Definition~\ref{def:numfuzz-interface} and show it satisfies the correctness
...
Our bounds analysis is inspired by interval analysis. For a concrete tuple 
$(r, a, b) \in \mathbb{P}$
we track using our set of bound $\bnd{\mathbb{B}}$ seven things:
...
\end{enumerate}
Accordingly, we define our $\bnd{\llbracket - \rrbracket}$ evaluation function:
\begin{equation}
  \begin{aligned}[c]
  \bnd{\llbracket} ((r^{\downarrow}, r^{\uparrow}), (a^{\downarrow}, a^{\uparrow}), (b^{\downarrow}, b^{\uparrow}), d) \bnd{\rrbracket} 
    = \\ \{ (r, a, b) \ | \
  ...
  \end{aligned}
\end{equation}
Our $\leq$ relation over $\mathbb{B}$ is simply the $\subseteq$ relation over
the interpretation of each bound:
\begin{definition}[$\leq$ over $\mathbb{B}$.]
  $b_0 \leq b_1 \iff \bnd{\llbracket} b_0 \bnd{\rrbracket} \subseteq
  \bnd{\llbracket} b_1 \bnd{\rrbracket}$
\end{definition}

We instantiate the modular interface with the interval operations $\Sigma = \{
\textbf{add}, \ \textbf{sub}, \ \textbf{mul} \}$ with the following types:
\begin{equation}
\begin{aligned}[c]
...
$\bnd{\mathbb{B}} \times \bnd{\mathbb{B}} \to \bnd{\mathbb{B}}$ as follows:
\begin{equation}
  \begin{aligned}[c]
    ...
    a^{\uparrow} b'^{\uparrow} + b^{\uparrow} a'^{\uparrow}), TODO) \\ 
  \end{aligned}
\end{equation}
where 
...
Given a rounding function $\rho_\mathbb{R}$ over the reals, we recall the
definition of our paired $\rho_{\mathbb{P}}$ below:
...
We need to show that our intantiatned bounds follow the required properties
specified in the modular interface.
...
($\mathbb{P}, (\mathbb{B}, \leq), d_{\mathbb{P}}, \rho_{\mathbb{P}}, u, \Sigma, \Sigma_{\mathbf{num}}$) 
satisfy the interface for instantiating Negative Fuzz specified in
...
Theorem~\ref{thm:paired-a-posteriori-rel}),
