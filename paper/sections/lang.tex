...
\AXC{$\bnd{\Delta \ |} \ \Gamma \vdash e : \tau_0$}
\RightLabel{(Op)}
\QIC{$\bnd{\Delta \ |} \ \Gamma \vdash \mathbf{op} ~ \bnd{\{i_0\}} ~ \bnd{\{i_1\}} ~ e : \tau_1$}
\bottomAlignProof
\DisplayProof
\vskip 1em
%%
...
\end{center}
    \caption{Typing rules for \Lang, with $s \in \NNR$ and $t,q,r,u \in \NNR
      \cup \{\infty\}$ and for $i \in \{ 1, 2 \}$ where $u$ is a fixed constant
      parameter (see Definition~\ref{def:numfuzz-interface} for details on
      picking an adequate constant). $FTV(\Gamma)$ refers to all the free type
      variables (e.g. $\epsilon_0, \epsilon_1$) in $\Gamma$.}
