...
Like many lambda calculi for linear type systems, we have variables, lambda
abstraction and application, $\pi_i$ for projection, $in_i$ for injection, and
two different types of products.
Our term syntax has explicit terms to represent our effects (rounding to the
nearest float: $\textbf{rnd}~e$) and coeffects (scaling sensitivity: $[e]$). For
...
types, and \textbf{let} for sequencing ordinary computation.
Note that our term language is more expressive than Numerical Fuzz as we can
bind and sequence expressions. 

Most exotically, we introduce a language for intervals (shown in
Figure~\ref{fig:syntax}) as well as explicit terms to represent polymorphic
abstraction $\bnd{(\Lambda \epsilon. e)}$ and instantiation $\bnd{(e \{i\})}$.
This is useful for incorporating a interval analysis into the type system, which
...
is $s$-sensitive in the expression $e$ and has round-off error upper-bounded by
$q$.
\begin{equation}
...
define pair and context summing and scaling below.
...
We are also able to scale typing contexts through the (! I) rule to instantiate
functions with finitely scaled senstivity.
...
principles for reasoning about round-off error. Note that $M_u E$ is the
characteristic rule that incorporates senstivity and round-off information.

A helpful perspective to view error grades on a type $M_q \tau$ is as a linear
resource; we describe this perspective in greater depth in
Section~\ref{sec:structure} and remark on how we can use the new (Factor) rule
to more richly structure and sequencing of arithmetic operations than prior
work. We use the richer structure to obtain tighet error analyses.

In this paper, we use the color $\bnd{\textit{blue}}$ to denote bound
polymorphism. 
In the (Widen), (\bnd{\textbf{bvar}}), (\bnd{\textbf{bop}}), ($\forall-I$), and
...
We additionally modify our (Op) typing rule from Numerical Fuzz to support bound
polymorphism. For presentational purposes, we present a restricted form of the
...
% ops
\AXC{$\bnd{\Delta \vdash i_0 : \textbf{bnd}}$}
\AXC{$\bnd{\Delta \vdash i_1 : \textbf{bnd}}$}
\AXC{$\{ \mathbf{op} : \bnd{\forall \epsilon_0, \epsilon_1,} \tau_0 \multimap \tau_1 \} \in \Sigma$}
\RightLabel{(Op)}
\TIC{$\bnd{\Delta \ |} \ \Gamma \vdash \mathbf{op} ~ \bnd{\{i_0\}} ~ \bnd{\{i_1\}} ~ v : \tau_1$}
\bottomAlignProof
\DisplayProof
\vskip 1em
%%
...
\begin{description}
  ...
  falling in same type relation $e_0, e_1 \in R_\tau$ (where $\tau$ is a closed
    type), we can write $e_0 \sim_r e_1 : \tau$ where $\mathcal{SD}_{\tau}(e_0,
    e_1) \leq r$. 
...
\begin{description}
  ...
  \item[c) Property of $\mathbf{op}$: metric preservation.] 
    ...
    v$ we have a corresponding function $op$ mapping bounds and syntactic values
    in
    $(\textbf{num}_{\epsilon_0} \otimes \textbf{num}_{\epsilon_1})$
    and
    $(\textbf{num}_{\epsilon_0} \times \textbf{num}_{\epsilon_1})$
    to $\textit{num}_i$ in a 1-sensitive manner as measured by $\mathcal{SD}$.
    Note that we could make this portion of the interface more general by
    definining it in terms of the logical relation below. We have opted to keep
    the interface minimal for presentational purposes.
