...
\begin{definition}[Evaluation of closed interval expressions]
  For an interval expression $b$,
  \begin{equation}
    \begin{aligned}[c]
      \llbracket (k_0, k_1) \rrbracket &\triangleq \{ k \ | \ \forall k \in \textit{num}, \ k_0 \leq k \leq k_1 \} \\
      \llbracket \textbf{iop}(b_0, \ldots, b_n) \rrbracket &\triangleq \textit{iop}(b_0, \ldots, b_n)
    \end{aligned}
  \end{equation}
\end{definition}
...
\begin{definition}[Interval expression equivalence class]
  ...
  $b_0 \sim b_1 \iff \llbracket b_0 \rrbracket =  \llbracket b_0 \rrbracket$ 
\end{definition}
