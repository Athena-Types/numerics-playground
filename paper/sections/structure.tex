...
We observe that the type $\textbf{factor}: (M_q \tau_0) \times (M_q \tau_1)
\multimap M_q (\tau_0 \times \tau_1)$ is neither derivable nor admissible (but
is proved sound in Theorem~\ref{thm:metric-preservation}). 
Adding \textbf{factor} as a primitive would allow for balanced computation
trees. 
