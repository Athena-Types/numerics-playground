...
bind rule in Numerical Fuzz forces the error to grow in $O(n)$ of the number of
added subterms.
To better understand why, we look to the type of the floating point addition
function:
...
We now describe an automatic program transformation $\textsc{transform}$ that
...
We first recusively transform the program so that every application is of a
variable to an expression (e.g. $e \ x$) or operation (e.g. $\textbf{op} \ y$). 
...
$\textsc{transform}$ syntatically operates on pairs of well-typed programs and
\textit{monadic maps} $\sigma$, which maps variables to expressions of monadic
type. It returns the transformed program fragment and sets of variables that
have not been used by factor. It has type:
% max: hmm this last part doesn't quite make sense wrt sets of vars
$$
\textsc{transform} : \text{program} \to (\text{var} \rightharpoonup \text{program}) \to
...
% \begin{equation}
%   \textsc{transform}(e) = \textsc{sequence}(\textsc{transform-rec}(e, \emptyset))
% \end{equation}
...
\begin{equation}
\begin{aligned}[c]
  \textsc{transform}(\textbf{let-bind} \ x \ = \ e \ \textbf{in} \ f, \sigma) &= 
  \begin{aligned}
    \text{let} \ (g, V) \ = (\textsc{transform}(f, \sigma;[x \mapsto \text{fst} \ \textsc{transform}(e, \sigma)])) \ \text{in} \\
    \text{if} \ x \in V \\
    \text{then} \ (\textbf{let-bind} \ x \ = \ e \ \textbf{in} \ g, V - \{x\})\\
    \text{else} \ (g, V)
\\
  ...
  \\
  \\
  \textsc{transform}(\textbf{let} \ z \ = \ \langle x, y \rangle \ \textbf{in} \ f, \sigma) &= 
  \begin{aligned}[c]
    \text{let} \ (g, V) = \textsc{transform}(f, \sigma) \ \text{in}\\
    \text{if} \ x, y \in DOM(\sigma) \\
    \text{then} \ (\textbf{let-bind} \ z = \ \textbf{factor} \ \langle \sigma[x], \sigma[y] \rangle \ \textbf{in} \ g, V - \{x, y\})\\
    \text{else} \ (\textbf{let} \ z \ = \ \langle x, y \rangle \ \textbf{in} \ g,
    V) \\
  \end{aligned} \\
  \\
  \textsc{transform}(e \ f, \sigma) &= 
  \begin{aligned}[c]
    \text{let} \ (g, V_0) = \textsc{transform}(e, \sigma) \ \text{in}\\
    \text{let} \ (h, V_1) = \textsc{transform}(f, \sigma) \ \text{in}\\
    (g \ h, V_0 \cap V_1)
  \end{aligned} \\
\end{aligned}
\end{equation}
Now, we detail our base case:
\begin{equation}
  \begin{aligned}
    \textsc{transform}(x, \sigma) &= (x, DOM(\sigma))
  \end{aligned}
\end{equation}
The remainder of the rules are recursive and structural.
