...
\textit{with} $\tau_1$; or equivalently, give the same error budget $M_q$ to
$\tau_0$ \textit{with} $\tau_1$.
...
particularly useful primitive to have for sequencing computations. 
...
running example. In the below figure, we demonstrate how the $\textbf{let-bind}$
rule interacts with our linear $\times$ to enable more efficiently structured
computation. 
\begin{figure}[ht]
\centering
