...
Now, we we can define $\textsc{factorize}$, which
syntatically operates on pairs of well-typed programs in adminstrative normal
form and \textit{monadic maps} $\sigma$, which maps variables to expressions of
monadic type. $\textsc{transform}$ returns the transformed program fragment and
sets of variables that have been used by factor. It has type:
