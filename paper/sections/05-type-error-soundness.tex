...
\end{theorem}
\begin{proof}
  We induct over our typing derivation. The base cases (Var, \textbf{bnd-var},
  Unit, Const) follow trivially. The subsumption case also follows trivially. We
  detail the remaining cases here. Note that the distance between $\textbf{bnd}$
  vars in $\Delta$ is $\infty$ if they correspond to different intervals.
  Therefore for the cases that do not touch $\Delta$ or $\textbf{bnd}$, we omit
  reptitively reasoning about $\Delta$ substitutions:
  \begin{description}
  ...
  \begin{description}
    \item[Case \textbf{bnd-op}.] Holds by the spec on our language interface and
