...
Our semantics models both our ideal and approximate numerical computations
simultaneously. This enables an explicit encoding of the computational content
of $\textbf{factor}$ (which can be viewed as rearranging various mixed
ideal/approximate intermediate computations) and multiplication in our monad
(which can be viewed as "forgetting" mixed intermediate computations through the
application of triangle inequality).

However, a semantics that unifies both ideal and approximate computations makes
it somewhat less obvious that our type soundness theorem is actually useful in
bounding round-off error: the difference between what the user wanted (a program
over the reals with no round-off error) and what the user got (a program over
the floats).
...
\begin{enumerate}
  \item For portions of the configuration that correspond to our \textit{ideal}
    computation, we \underline{underline} the computation. For example, our
    rounding stepping rule would look like:
    \begin{equation*}
  ...
  \item For portions of the configuration that correspond to our
    \textit{approximate} computation, we $\overline{\text{overline}}$ the
    computation. For example, our rounding stepping rule would look like:
    \begin{equation*}
    ...
    \end{equation*}
\end{enumerate}
A user may choose to selectively consider the ideal semantics by only caring
about the underlined portions of the operational semantics rewrite rules, or, a
user may selectively implement the approximate floating-point semantics by
computing the overlined portions of the operational semantics rewrite rules.
Using annotations, we prove our error soundness theorem
...
component side fully ideal (what the user wanted) and the second component fully
approximate (what the user got and can compute in-hardware), with the distance
between computations --- our round-off error --- bounded above by $q$.
...
\subsubsection{Well-annotated relation}
We will then define a unary well-annotated configuration relation and prove that
it is preserved under stepping. 

\subsubsection{Error soundness}

The following lemma proves that when the expression on the right-hand side of a
$\Vdash$ relation is simulatenously under-over-lined, the property is preserved
under stepping (Lemma~\ref{thm:expressions-preserve-annotations}).
\begin{lemma}\label{thm:expressions-preserve-annotations}
For all $\sigma, \sigma', e, e'$, if
$\sigma \Vdash e \rightsquigarrow \sigma' \Vdash e'$, 
  then 
  $\sigma \Vdash \underline{\overline{e}} \ \textcolor{red}{\rightsquigarrow} \ \sigma' \Vdash \underline{\overline{e'}}$. 
\end{lemma}
\begin{proof}
  By inspection of the annotated rewrite ($\rightsquigarrow$) relation.
\end{proof}
