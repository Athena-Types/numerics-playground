...
\begin{proof}
  ...
  implication case, we observe the following: TODO.
\end{proof}
...
\begin{description}
  ...
  \item[\textsc{(ty. rule) $\multimap I$.}] We are given in our inductive
    ...
    To show this, we need to demonstrate that for any $y \in \llbracket \tau_0
    \rrbracket$, we need show that for all $\llbracket \sigma \Vdash \lambda x . e
    \rrbracket_{\tau_0 \multimap \tau_1} y \in \llbracket \tau \rrbracket$.

    Let $\sigma$ be an arbitrary enviroment compatible with $\Gamma$. 
