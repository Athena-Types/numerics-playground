  ...
  \item[\textsc{(ty. rule) Var.}] There is no premise containing a typing
    ...
    \begin{description}
      \item[\textit{(env. size) 0.}] We observe that an enviorment size of zero
        ...
        \rrbracket$$
        for $\sigma$ compatible with $\Gamma, x : \tau$. We proceed to case over
        the type of our variable, $\tau$.
        \begin{description}
          ...
          \item{\underline{(ty. case) \textbf{num.}}} The only configurations compatible
