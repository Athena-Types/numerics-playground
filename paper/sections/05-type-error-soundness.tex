...
\begin{lemma}[$r$-sensitive subsitution]\label{thm:substitution}
  ...
  $\Delta_0 + \Delta_1 \ | \ r \cdot \Gamma + \Theta \vdash e'[e/x] : \tau'$.
\end{lemma}
\begin{proof}
  We first induct over the height of the second typing derivation. Note that the
  $\bnd{Bvar}$ and $\bnd{Bop}$ rules are rules for a different typing judgement
  from our main judgement $\vdash$.
  \begin{description}
    \item[Base cases (unit, var, Const).] Follows by application of the first typing rule wherever
      the var $x$ is used.
    \item[Inductive case.]
      For each premise in the typing rule, by our inductive hypothesis, we can
