...
two key properties of each function: the function sensitivity $s$ and the
maximum error $q$ introduced by the function. Intuitively, $s$ upper-bounds the
amount by which perturbations in a function's input affect its output. Formally,
function sensitivity is defined below.
In this paper, for every symbol $\phi$, $\tilde{\phi}$ represents the
floating-point approximation of $\phi$.

\begin{definition}[Lipschitz function senstivity]
...
\tilde{a} - \tilde{b}$. We detail two approaches towards covert relative bounds
...
\begin{description}
  \item[1) \textit{A priori.}] If we can obtain interval bounds on the paired
    ideal components $a$ and $b$, we would be able to statically produce bounds
    on the distance beteen $r$ and $\tilde{r}$.
...
accomplish this, we incorporate an interval bounds analysis in the type system.
A challenge of doing interval analysis in the type system is how to assign a
precise interval bound to a function that has multiple call sites. A na{\"i}ve
approach might widen the intervals for each call site, drastically reducing the
precision of the interval analysis. We address this challenge by introducing
interval \textit{bound polymorphism}, allowing functions to be polymorphic in
the interval bounds assigned to the inputs. At each call site, we specialize the
polymorphic function to the appropriate intervals. 
...
\begin{itemize}
  ...
  \item We instantiate Numerical Fuzz to use a \textit{paired representation}
    ...
    in a finitely-sensitive manner (Section~\ref{sec:instantiation}). We prove
    that error types can be used to provide both \textit{a priori} and \textit{a
    posterori} compositional error bounds in the presence of subtraction and
    negative numbers (Section~\ref{sec:application}). 
