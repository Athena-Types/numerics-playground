...
We use this representation combined with an intervals bounds analysis
...
We defer the details of this approach to Section \ref{sec:encoding}. 

We address the above limitations by providing a scalable, type-based
...
\begin{itemize}
  \item 
    We extend the Numerical Fuzz family of languages by incorporating an
    interval-style analysis into the type system using interval \textit{bound
    polymorphism}. We introduce a previously-untypable primitive that enables
    the error and sensitivities in rounded terms to be shared, allowing for
    tighter error bounds. We further extend Numerical Fuzz to enable expressions
    in more places. We prove the soundness of our extensions to Numerical Fuzz
    in (Section~\ref{sec:lang}).
  ...
  \item We encode Numerical Fuzz to use a \textit{paired representation} that
    ...
    We then prove both \textit{a priori} and \textit{a posterori} compositional
