...
handling negative numbers and subtraction. A crucial component of prior work was
to track function Lipschitz sensitivity, which bounds the amount a function can
amplify round-off error. The fundamental problem with adding negative numbers
and subtraction is that those operations are infinitely-sensitive; relatively
small peturbations in the input can lead to hard-to-bound relatively large
perturbations in the output. As an illustrative toy example, suppose we have two
large numbers like 1,000,001 and 1,000,000 that we wish to subtract. If we
perturb the input by a small amount, for example, by 1 per-cent, we can observe
quite large \textit{relative} changes in the output. 
